...
func (*server) ListFiles(ctx context.Context, req *pb.ListFilesRequest) (*pb.ListFilesResponse, error) {
	...
	dir := "/Users/suzu/GolandProjects/awesomeProject/lesson2/storage"
...
func (*server) Download(req *pb.DownloadRequest, stream pb.FileService_DownloadServer) error {
	...
	path := "/Users/suzu/GolandProjects/awesomeProject/lesson2/storage/" + filename
...
func (*server) Upload(stream pb.FileService_UploadServer) error {
	...
	for {
		...
		data := req.GetData()
		log.Panicf("reveived data(bytes): %v", data)
		log.Panicf("reveived data(string): %v", string(data))
		buf.Write(data)
	}
}
...
func myLogging() grpc.UnaryServerInterceptor {
	return func(
		...
		log.Printf("response datra: %v", resp)
